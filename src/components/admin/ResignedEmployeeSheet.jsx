...
const ResignedEmployeeSheet = () => {
    ...
    const fetchAppData = async () => {
        try {
            ...
            const [entriesRes, membersRes, deptsRes] = await Promise.all([
                axios.get(`${API_ORIGIN}/api/resigned-employees/entries?month=${month}&year=${year}`, axiosConfig),
                axios.get(`${API_ORIGIN}/api/hr/members`, axiosConfig),
                axios.get(`${API_ORIGIN}/api/departments`, axiosConfig)
            ]);

            let fetched = entriesRes.data || [];

            if (!Array.isArray(fetched) || fetched.length === 0) {
                fetched = createFallbackRows();
            }
    ...
    const deleteRow = async (idx) => {
        ...
        if (entry._id || entry.id) {
            ...
            try {
                ...
                await axios.delete(`${API_ORIGIN}/api/resigned-employees/entry/${entry._id || entry.id}`, {
                    headers: { 'x-auth-token': token }
                });
    ...
    const saveEntry = async (idx) => {
        ...
        try {
            ...
            const res = await axios.post(`${API_ORIGIN}/api/resigned-employees/entry`, {
                month,
                year,
                entry
            }, {
                    ...
                    <div className="flex flex-col items-end">
                        ...
                        <div className="flex gap-2">
                            {[
                                ...
                                <span
                                    key={s.label}
                                    className={`px-2 py-0.5 rounded-md text-[9px] font-black border uppercase ${s.color}`}
                                >
                                    {s.label}
                                </span>
                            ))}
                        ...
                        <thead>
                            <tr className="bg-slate-900/80 border-b border-white/5">
                                ...
                                {taskColumns.map(tc => (
                                    <th
                                        key={tc.key}
                                        className="p-4 text-[10px] font-black text-slate-500 uppercase tracking-widest text-center w-28"
                                    >
                                        {tc.label}
                                    </th>
                                ))}
                                ...
                                <th className="p-4 w-20 text-center text-[10px] font-black text-slate-500 uppercase tracking-widest">
                                    Saved
                                </th>
                            </tr>
                        </thead>
                                            ...
                                            <button
                                                onClick={() =>
                                                    handleTaskChange(
                                                        idx,
                                                        tc.key,
                                                        entry.tasks && entry.tasks[tc.key] === 'Done' ? 'Pending' : 'Done'
                                                    )
                                                }
